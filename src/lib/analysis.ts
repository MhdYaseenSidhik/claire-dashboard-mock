...
export async function fetchAnalysis(url: string = DATA_URL, signal?: AbortSignal): Promise<AnalysisBundle> {
  ...
  if (!data || !data.kpis || !Array.isArray(data.forecasts) || !Array.isArray(data.alerts)) {
    throw new Error('Analysis bundle is malformed — expected kpis, forecasts and alerts.');
  }
