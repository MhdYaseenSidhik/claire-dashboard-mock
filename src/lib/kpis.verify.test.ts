 ...
 *
 * STATUS AT TIME OF WRITING: the upstream INV-4 module and the extract JSON are
 * NOT YET in the repo (S-4 build is in_progress/blocked, workspace empty, sandbox
 * cannot fork so no checkout/run was possible). The imports below name the exact
 * contract QA expects. When the module + data land, this suite runs unchanged via
 * `npm test` / sandbox_run_tests. Until then it fails at import — which is the
 * correct, honest signal that there is nothing to verify yet.
 *
